...
(defclass ship ()
  ((name
   ...
   (cruise
    :initform 0
    :initarg :cruise
    :accessor cruise) ; shall be transient
   (dinghy
    :initform NIL
    :initarg :dinghy
    :accessor dinghy)) ; another ship -> ref
...
(defclass dinghy (ship)
  ((aboard
    :initform NIL
    :initarg :aboard
    :accessor aboard))) ; another ship -> circular ref
...
(defmethod set-up ((self basetest))
  ...
  (setf (ship2 self)
	(make-instance 'sailingship
		       :name      "Pinta"
		       :course     270
		       :cruise     9
		       :dimensions '(:width 7 :length 21)
		       :sailarea   400))
  (setf (ship3 self)
	(make-instance 'motorship
		       :name       "Titanic"
		       :course     320
		       :cruise     21
		       :dimensions '(:width 28 :length 269)
		       :enginepower 51000))
  (setf (ship4 self)
	(make-instance 'motorsailor
		       :name       "Gorch Fock"
		       :course      180
		       :cruise      18
		       :dimensions  '(:width 12 :length 82)
		       :sailarea    2037
		       :enginepower 1660))
  (setf (ship5 self)
	(make-instance 'dinghy
		       :name       "Gig"
		       :course     320
		       :cruise     5
  ...
  (setf (ship6 self)
	(make-instance 'dinghy
		       ...
		       :dimensions '(:width 2 :length 6)
		       :aboard     (ship4 self)))
  (setf (dinghy (ship3 self))
	(ship5 self))  ; ref only
  (setf (dinghy (ship4 self))
	(ship6 self))  ; -> circle
  (setf (ships self)
	(list (ship1 self)
	      (ship2 self)
...
(def-test-method objectarraytest ((self typestest) :run nil)
  (let* ((arr   (make-array '(2 3)
			    :initial-contents (list (list (ship1 self) (ship2 self) (ship3 self))

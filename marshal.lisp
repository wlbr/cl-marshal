...
(defmethod marshal :around (thing &optional (circle-hash nil))
   (if circle-hash
      ...
      (progn
        ...
        (list (coding-idiom :coding-identifier)
	      (coding-idiom :coding-release-no)
	      (call-next-method thing circle-hash)))))
   ...
   (let* ((ckey        nil)
      ...
      (if ckey
         ...
         (progn
		  ...
		  do (setf output
			   (nconc output
				  (list (marshal walker circle-hash))))))
           (push ckey output)
           (push (if dotted-list
                    (coding-idiom :dlist)
                    (coding-idiom :list))
   ...
   (let* ((ckey   nil)
      ...
      (if ckey
         ...
         (progn
           ...
           (setq output
		 (list (coding-idiom :array) ckey
		       (array-dimensions array)
		       (array-element-type array)))
           (dotimes (walker (array-total-size array))
	     (push (marshal (row-major-aref array walker)
			    circle-hash)
		   dummy))
           (setq output
		 (nconc output
   ...
   (let* ((ckey   nil)
      ...
      (if ckey
         ...
         (progn
           ...
           (setq output (list (coding-idiom :simple-string)
			      ckey
			      object))))
      output))
   ...
   (let* ((ckey   nil)
      ...
      (if ckey
         ...
         (let ((fill-pointer       (fill-pointer object))
            ...
            (setq output (list (coding-idiom :string)
			       ckey
			       fill-pointer
			       adjustable-array-p
   ...
   (let* ((ckey   nil)
      ...
      (if ckey
         ...
         (progn
           ...
           (setq output (list (coding-idiom :hash-table)
			      ckey
